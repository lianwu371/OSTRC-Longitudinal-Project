...
def create_model(optimizer='adam', learn_rate=0.01, momentum=0, init_mode='uniform', activation='relu', dropout_rate=0.0, weight_constraint=0, neurons=1):
	# create model
	model = Sequential()
	model.add(Dense(neurons, input_dim=71, kernel_initializer=init_mode, activation=activation, kernel_constraint=maxnorm(weight_constraint)))
...
best.to_csv(r'C:\Users\Lian\OneDrive\Phase 4(With new data) - 27.03.2020\Script\Parameters\'ANN_unprocessed.csv')
...
model = Sequential()
model.add(Dense(40, input_dim=71, kernel_initializer='uniform', activation='tanh', kernel_constraint=maxnorm(7)))
