...
best.to_csv(r'C:\Users\Lian\OneDrive\Phase 4(With new data) - 27.03.2020\Script\Parameters\'RD_unprocessed.csv')

#Sliding window actualy improved the accuracy of the model, next try to impliment forward method and see.

#%%#-----------------------Random Forest-specialised model----------------------------------------------
from sklearn.model_selection import train_test_split
import pandas as pd
import numpy as np
...
features_used_RF = RF_model.feature_importances_
x_axis = np.arange(len(features_used_RF))

import matplotlib as mpl
mpl.rcParams['figure.dpi'] = 300
plt.figure(figsize=(9,3))
plt.plot(x_axis, features_used_RF)
plt.title('Feature Importance of RF using Data with no Time dependency Method')
...
y_label = [79.10, 79.49, 78.71, 78.91, 78.52] #accuracy
x_label = [80, 90, 100, 110, 120 ] #hyperparameter, max_depth
mpl.rcParams['figure.dpi'] = 300
plt.figure(figsize=(9,3))
plt.plot(x_label, y_label)
plt.title('RF Model Prediction VS max_depth')
plt.ylabel('Accuracy')
