...
#------------------------------------Data structure reconfiguration--------------------------------------------------
adding_1 = []
adding_2 = []
adding_3 = []
...
for i in range(1, df.shape[0]):
    ...
    adding_1.append(data_entry[2])
    adding_2.append(data_entry[3])
    adding_3.append(data_entry[4])

df = df.head(df.shape[0]-1)

df["Sport-specific"] = adding_1
df["Strengthfitness"] = adding_2
df["Competition"] = adding_3
...
def lstm_walking(df):
    ...
    #Remove unnecessary columns
    #walking_forward(data, n, personalised , athelete_name):
    #X,y = walking_forward(df, len(df), 0, 0.0 )
    X,y = walking_forward(df, 100, 0, 0.0 )
    model_score = []
    ...
    for i in range(1,len(X), 10):
        ...
        test_X = test_X1.tail(1)
        test_y = test_y1.tail(1)
        ...
        model.fit(train_X, train_y, epochs=5, batch_size=20, verbose=1)
        print(model.summary())
        
        scores = model.evaluate(test_X, test_y, verbose=1)
        model_score.append(scores[1]*100)
        print("Accuracy: %.2f%%" % (scores[1]*100))
        #you either get it right or get it wrong, there is no middle ground
    ...
    return model, X, y, test_X, test_y, train_X, train_y, model_score

model, X, y, test_X, test_y, train_X, train_y, model_score = lstm_walking(df)
...
#%%#-----------------------Tuning----------------------------
#% seems to be working, implimenting lstm for data forwading
...
def create_model(optimizer='adam', learn_rate=0.01, momentum=0, dropout_rate=0.0, neurons=1):
	# create model
    #5 means the number of LSTM blocks or neurons
    model = Sequential()
...
neurons = [20, 25, 30]
param_grid = dict(batch_size=batch_size, epochs=epochs, optimizer=optimizer, learn_rate=learn_rate, momentum=momentum, dropout_rate=dropout_rate, neurons=neurons)
grid = GridSearchCV(estimator=model, param_grid=param_grid, n_jobs=-1, cv=3)

#This is where is gets wrong, the input dimensions are wrong, adjust for the lstm input structure

X,y = walking_forward(df, 2045, 0, 0.0 )
...
best.to_csv(r'C:\Users\Lian\OneDrive\Phase 4(With new data) - 27.03.2020\Script\Parameters\'LSTM_forward_chaining.csv')

#%%#-----------------------LSTM forward chaining-specialised model----------------------------------------------
...
for i in range(1,len(X)):
    ...
    if i == 2:
        model.fit(train_X, train_Y, epochs=100, batch_size=60, verbose=1)
    else: 
...
dropout_rate = [0.5, 0.6, 0.7]
neurons = [25, 30, 40]
param_grid = dict(batch_size=batch_size, epochs=epochs, optimizer=optimizer, lr=learn_rate, momentum=momentum, dropout_rate=dropout_rate, neurons=neurons)
grid = GridSearchCV(estimator=model, param_grid=param_grid, n_jobs=-1, cv=3)
...
best.to_csv(r'C:\Users\Lian\OneDrive\Phase 4(With new data) - 27.03.2020\Script\Parameters\'LSTM_data_sliding.csv')

#%%#-----------------------LSTM data sliding-specialised model----------------------------------------------
...
history = model.fit(train_X, train_y, epochs=50, batch_size=40, validation_data=(test_X,test_y), verbose=1)
